...
export function initializeDatabase(): void {
  ...
  const migrations = [
    ...
    ['contact', 'email', 'admin@darcyaviation.com', 'admin@darcyaviation.com'],
  ...
  const replaceContentEmail = db.prepare('UPDATE site_content SET content = replace(content, ?, ?), updated_at = CURRENT_TIMESTAMP WHERE content LIKE ?');
  const replaceMetaEmail = db.prepare('UPDATE pages_meta SET description = replace(description, ?, ?), updated_at = CURRENT_TIMESTAMP WHERE description LIKE ?');

  const runMigrations = db.transaction(() => {
    for (const [section, key, oldVal, newVal] of migrations) {
      migrateContent.run(newVal, section, key, oldVal);
    }
    replaceContentEmail.run('brent@darcyaviation.com', 'admin@darcyaviation.com', '%brent@darcyaviation.com%');
    replaceMetaEmail.run('brent@darcyaviation.com', 'admin@darcyaviation.com', '%brent@darcyaviation.com%');
